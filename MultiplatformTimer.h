/**
 * Copyright(c) 2013, jgoenetxea
 * All Rights Reserved.
 */

#ifndef MULTIPLATFORM_TIMER_H_
#define MULTIPLATFORM_TIMER_H_
...
public:
    ...
    double getElapsedSeconds(std::string msg = "") {
		long long current;
        Timer::QueryCounter(&current);
		double secs = static_cast<double>(current - m_init) / usec_per_msec ;
#ifdef __MACH__
        secs *= 0.001f;
#elif __unix__
        secs *= 0.001f;
    ...
    double getTotalElapsedSeconds(std::string msg = "") {
		long long current;
        Timer::QueryCounter(&current);
		double secs = static_cast<double>(current - m_global) / usec_per_msec ;
		m_init = current;
		m_global = current;
#ifdef __MACH__
        secs *= 0.001f;
